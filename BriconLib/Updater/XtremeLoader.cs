...
// Type: BriconLib.Updater.XtremeLoader
...
{
  internal class XtremeLoader
  {
    public static void TryToLoad(CommunicationForm commFrm)
    {
      bool flag1 = false;
      ...
      foreach (SoftwareVersionInfo version in SoftwareDownloader.Versions)
      {
        if (version.VersionType == SoftwareVersionInfo.VersionTypes.Xtreme && !flag1 || version.VersionType == SoftwareVersionInfo.VersionTypes.SpeedyXtreme && flag1)
        {
          ...
          {
            versionTo = version.MenuString;
            str = version.LocalFileName;
            flag2 = true;
            try
            {
      ...
      if (new LoadingXtreme(CommunicationState.Instance.BAVersion.Substring(3), versionTo).ShowDialog() != DialogResult.OK || MessageBox.Show(ml.ml_string(1258, "Make sure that you have read the pigeons out of the clock before you continue, since all data will be lost, continue?"), ml.ml_string(1259, "Loading Xtreme"), MessageBoxButtons.YesNo) == DialogResult.No)
        return;
      int num1 = 70;
      commFrm.SecondsRemaining = num1;
      ...
      try
      {
        ...
        Process process = Process.Start(new ProcessStartInfo("Xtreme.bin", "-dXtreme -e -if" + str.Replace("Updates\\", "") + " -pf -vf -X -cCOM" + activeComport.ToString())
        {
